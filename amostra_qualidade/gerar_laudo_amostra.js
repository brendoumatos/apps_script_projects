function gerar_laudo(){
  ...
  var guia_amosta_avaliada = planilha.getSheetByName("Amostras_Avaliadas")
  var guia_templante = planilha.getSheetByName("Template")
  var pastaFoto = DriveApp.getFolderById("10F4MMoFIwx-7ztw23EJ7X541tB4oBf--")
  ...
  var lr_laudo_gerado = guia_amosta_avaliada.getRange(1,3).getNextDataCell(SpreadsheetApp.Direction.DOWN).getRowIndex();
  var lr_laudo_enviado = guia_amosta_avaliada.getRange(1,2).getNextDataCell(SpreadsheetApp.Direction.DOWN).getRowIndex();
  ...
  for(var linha_atual = lr_laudo_enviado; linha_atual <= lr_laudo_gerado; linha_atual++){
    ...
    var lr_id_gerado = guia_amosta_avaliada.getRange(1,1).getNextDataCell(SpreadsheetApp.Direction.DOWN).getRowIndex();
    var ultimo_id_amostra = guia_amosta_avaliada.getRange(lr_id_gerado, 1).getValue();
    var id_linha_atual = guia_amosta_avaliada.getRange(linha_atual, 1).getValue();

    if(id_linha_atual == ''){
      
      var id_extraido_amostra = ultimo_id_amostra.split(" / ");
      var id_amostra = Number(id_extraido_amostra[0]);
      var ano_atual = guia_amosta_avaliada.getRange(linha_atual, 3).getValue().getFullYear();
      
      var novo_id = (id_amostra + 1) + " / " + ano_atual;
     
      
      guia_amosta_avaliada.getRange(linha_atual, 1).setValue(novo_id);
    ...
    var status_laudo = guia_amosta_avaliada.getRange(linha_atual, 2).getValue();
    ...
    if(status_laudo == ''){ //se o status estiver vazio, gerar laudo
      ...
      //verificar se há foto ativa e buscar id
        ...
        var evidencia01 = guia_amosta_avaliada.getRange("J" + linha_atual).getValue();
        var evidencia02 = guia_amosta_avaliada.getRange("N" + linha_atual).getValue();
        var evidencia03 = guia_amosta_avaliada.getRange("R" + linha_atual).getValue();
        var evidencia04 = guia_amosta_avaliada.getRange("V" + linha_atual).getValue();
        
        if(evidencia01 != ''){ //if evidencia01 is not null extract values

          var nome_foto01 = evidencia01.toString().split("/");//quebrar texto
      ...
      if(evidencia02 != ""){ //if evidencia02 is not null extract values
          ...
          var nome_foto02 = evidencia02.toString().split("/");//quebrar texto
      ...
      var area_solicitante = guia_amosta_avaliada.getRange("F" + linha_atual).getValue();//captura area que enviou amostra
      var fornecedor = guia_amosta_avaliada.getRange("AD" + linha_atual).getValue();//captura o nome do fornecedor
      ...
      var ordem = guia_amosta_avaliada.getRange("E" + linha_atual).getValue();//captura numerio da ordem
      ...
      //gerando pdf
      var numeroOrdem = guia_amosta_avaliada.getRange("E" + linha_atual).getValue();
      let mensagem = {
        ...
        name: "qualidade.total@ciahering.com.br",
      ...
      guia_amosta_avaliada.getRange("B" + linha_atual).setValue("Enviado");//informando que o laudo foi enviado
