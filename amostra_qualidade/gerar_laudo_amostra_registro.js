function gerar_laudo_amostra_registro_area(){
  ...
  var pastaFoto = DriveApp.getFolderById("10F4MMoFIwx-7ztw23EJ7X541tB4oBf--") //1srENddDFqJoTIjutOuXC9w3zwNzpgx7s    10F4MMoFIwx-7ztw23EJ7X541tB4oBf--
  var tabReferencias = planilha.getSheetByName("Tab Referencia")
  var guia_lista_email = planilha.getSheetByName("ListaEmail")
  ...
  for(var linha_atual = lr_laudo_enviado; linha_atual <= lr_laudo_gerado; linha_atual++){
      ...
      if(status_laudo == ''){ //se o status estiver vazio, gerar laudo
        ...
        //verificar se há foto ativa e buscar id
          ...
          if(evidencia01 != ''){ //if evidencia01 is not null extract values
            ...
            var nome_foto01 = evidencia01.toString().split("/Amostras_Registro_Images//");//quebrar texto
        ...
        if(evidencia02 != ""){ //if evidencia02 is not null extract values
            ...
            var nome_foto02 = evidencia02.toString().split("/Amostras_Registro_Images//");//quebrar texto
          ...
          if(evidencia03 != ""){ //if evidencia03 is not null extract values
            ...
            var nome_foto03 = evidencia03.toString().split("/Amostras_Registro_Images//");//quebrar texto
          ...
          if(evidencia04 != ""){ //if evidencia04 is not null extract values
            ...
            var nome_foto04 = evidencia04.toString().split("/Amostras_Registro_Images//");//quebrar texto
            ...
            guia_templante.getRange("H17").setFormula('=HYPERLINK("https://drive.google.com/file/d/' + id_foto04 +'";4)')
        ...
        guia_lista_email.getRange("A1").setFormula("=" + idPlanilhaFornecedor); //seta planilha com nome dos inspetores
        ...
        Logger.log("ETAPA 07.1.1 - Validando se é fornecedor interno")
          ...
          var conf_interna = area_solicitante.toString().indexOf("Confecção Interna") > -1;

          Logger.log(conf_interna)

          if( conf_interna == true){
            
            let turno_fornecedor = guia_amostra_registro.getRange("AI" + linha_atual).getValue(); // captura o turno do fornecedor

            gpFornecedor = gpFornecedor + "-" + turno_fornecedor;
            
          }

        Logger.log("ETAPA 07.2 - Bucando email dos envolvidos")
        //rodar linha por linha buscando o grupo do fornecedor
        for( var linhaFornecedorRef = 1; linhaFornecedorRef < guia_lista_email.getRange("B1").getNextDataCell(SpreadsheetApp.Direction.DOWN).getRowIndex() ;linhaFornecedorRef++ ){
            
          var fornecedorRef = guia_lista_email.getRange("A" + linhaFornecedorRef).getValue(); //captura o grupo atual

          //testa se encontrou o fornecedor
          if(fornecedorRef == gpFornecedor || fornecedorRef == undefined){
            ...
            if(conf_interna == true){
              ...
              var emailAnalista = '';
              var emailInspetor = guia_lista_email.getRange("D" + linhaFornecedorRef).getValue();
            
            }else{

              var emailAnalista = guia_lista_email.getRange("C" + linhaFornecedorRef).getValue();
              var emailInspetor = guia_lista_email.getRange("D" + linhaFornecedorRef).getValue();
              var emailFornecedor = guia_lista_email.getRange("E" + linhaFornecedorRef).getValue();
            ...
            var emailDestinatario = listaEmails
              .filter(function(e) {
                return e &&
                    e.toString().trim() !== "" && 
            ...
            // não encontrou grupo, ignora email dos responsáveis
            emailDestinatario = ''
          }

        }
        Logger.log(emailDestinatario);
        ...
        var avaliador_amostra = guia_amostra_registro.getRange("D" + linha_atual).getValue();//captura o nome do avaliador


        //setar modelo email para area especifica (confeccao ou nao?)
        var areaConfeccao = area_solicitante.indexOf("Confecção");
        Logger.log(areaConfeccao);
        
        areaConfeccao = areaConfeccao > -1
        ...
        if(areaConfeccao == true){
          ...
          mensagem = {
            to:emailFixo + "," + emailDestinatario,
            cc: emailTimeQualidade + "," + avaliador_amostra,
            ...
            htmlBody: "Olá time!<br><br>" +
              "Por meio deste, informamos que a amostra <b>" + idParaLaudo + "</b>, referente à ordem: <b>" + ordem + "</b>, foi avaliada e consta no relatório em anexo e, também no aplicativo " +
              "<a href='https://www.appsheet.com/start/5392113f-32a6-4696-ac82-ee601fa5175b?platform=mobile#appName=Quality-1001230722&vss=H4sIAAAAAAAAA6WSwU6DQBCGX8XMmTa0ldTszaiHxmiiEg-KaUZ2MBsXlsCiEsK7O2BrGyUp0OP-y_fvzBcq-FD0-WAxfAfxXO1O11SCgCoAv0wpABHAhUlsZnQATgC3GP-EPmk8WSUqVMg3NdTO8I6YkmJ9HpvcZji2Y4Pn67sCtZIoqW16cbZNlnIQ1fCFxPFOHFCSEqsiRVnT19Dcs2H5uiE52OdYAsSFxVdN7fDM1RwNdjli_L8Vfcff5zrG3wJdXztwb4zl9A21pqzk4BItMhqnnM7duTeZuZOZ57uu8OZisZyeet4T9FLS_WuMENNd1FfPf_qwpA7mV1WImTzo6Wy6XLSe-KnIhEVO8pG3PmrbfJVcfaWYyBvD84gIdU71NyQYr-pIBAAA&view=Amostras_Qualidade'>" +
              "Quality</a>.<br><br>" +
          ...
          mensagem = {
            ...
            subject: "Amostra de Qualidade: " + idParaLaudo + " - Op: " + ordem,
            htmlBody: "Olá time!<br><br>" +
              "Por meio deste, informamos que a amostra <b>" + idParaLaudo + "</b>, referente à ordem: <b>" + ordem + "</b>, foi devidamento avaliada e encontra-se no relatório em anexo, bem como disponível no aplicativo " +
              "<a href='https://www.appsheet.com/start/5392113f-32a6-4696-ac82-ee601fa5175b?platform=mobile#appName=Quality-1001230722&vss=H4sIAAAAAAAAA6WSwU6DQBCGX8XMmTa0ldTszaiHxmiiEg-KaUZ2MBsXlsCiEsK7O2BrGyUp0OP-y_fvzBcq-FD0-WAxfAfxXO1O11SCgCoAv0wpABHAhUlsZnQATgC3GP-EPmk8WSUqVMg3NdTO8I6YkmJ9HpvcZji2Y4Pn67sCtZIoqW16cbZNlnIQ1fCFxPFOHFCSEqsiRVnT19Dcs2H5uiE52OdYAsSFxVdN7fDM1RwNdjli_L8Vfcff5zrG3wJdXztwb4zl9A21pqzk4BItMhqnnM7duTeZuZOZ57uu8OZisZyeet4T9FLS_WuMENNd1FfPf_qwpA7mV1WImTzo6Wy6XLSe-KnIhEVO8pG3PmrbfJVcfaWYyBvD84gIdU71NyQYr-pIBAAA&view=Amostras_Qualidade'>" +
              "<Quality/a>.<br><br>" +
              "<br>"+
    ...
    guia_templante.getRange("B16:M17").clearContent();
    guia_lista_email.getRange("A1").clearContent();
    tabReferencias.getRange("E3").clearContent();
    tabReferencias.getRange("E4").clearContent();
    
    }catch(e){

      Logger.log("Ocorreu um erro ao gerar o laudo na linha:" + linha_atual)

      console.log("O erro foi:" + e)

      var msg_erro = {
            to:avaliador_amostra,
            subject: "ERRO | Amos de Qualidade - Op: " + ordem,
            htmlBody: "Olá!<br><br>" +
              "Identificamos um erro na amostra de qualidade da ordem: <b>" + ordem + "</b>, e por isto, não conseguimos enviar o email. Pedimos que acesse os detalhes dessa amostra para apuração da falha, no aplicativo" +
